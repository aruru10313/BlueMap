...
### 3. 디스크 영구 저장 & 비동기 배치 플러시 (Timelapse Persistence)
- **서버 재부팅 후에도 건축 기록 유지**:
  - `data/timelapse/metadata.json` 및 `data/timelapse/{dimension}.jsonl`에 블록 이벤트가 자동 영구 저장됩니다.
  - 서버를 재시작해도 최초 서버 시작 시간(`serverStartTime`)과 단조 증가 시퀀스(`seqGenerator`), 과거 건축 타임라인이 100% 온전히 복원됩니다.
- **서버 틱 지연 0ms (비동기 큐 & 주기적 배치 플러시)**:
  - 블록 설치 시 메인 틱 스레드에서는 `ConcurrentLinkedQueue`에만 등록하고, 백그라운드 데몬 스레드가 2초마다 일괄 파일 플러시를 수행하여 마인크래프트 서버 TPS 지연이 전혀 발생하지 않습니다.
  - 서버 저장(`/save-all`) 및 종료(`/stop`) 시 잔여 큐가 자동으로 안전하게 플러시됩니다.

### 4. 페이지 새로고침 없는 실시간 동기화 (Auto-Redraw & Sync)
